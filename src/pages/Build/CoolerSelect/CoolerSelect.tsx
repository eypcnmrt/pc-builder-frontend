...
import useCoolerSelect from "../../../hooks/useCoolerSelect";
...
const CoolerSelect = () => {
  ...
  const renderOptionLabel = (item: Cooler) => {
    const compat = processorSocket && isCompatible(item) ? " [Uyumlu]" : "";
    return `${item.brand} ${item.model} — ${item.type} · TDP: ${item.tdpW}W${item.heightMm ? ` · ${item.heightMm}mm` : ""}${compat}`;
  };
          ...
          <div className="grid grid-cols-[280px_1fr] gap-6 items-start">
            ...
            <div className="panel accent-top animate-fade-in-up p-5">
              ...
              {processorSocket && (
                <div className="mb-4 py-2 px-[10px] bg-[rgba(6,182,212,0.08)] border border-[rgba(6,182,212,0.3)]">
                  ...
                  <div className="font-mono text-[0.55rem] text-[var(--color-text-dim)] mt-0.5">
                    Uyumlu soğutucular listenin başına alındı
                  </div>
                </div>
              )}
              ...
              <RangeRow
                ...
                onChange={(v) => updateFilter("tdpRange", v)}
                format={(n) => `${n}W`}
              />
            </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="panel accent-top corner-brackets animate-fade-in-up stagger-2 p-6">
                <label className="font-mono flex items-center gap-[6px] text-[0.65rem] tracking-[0.14em] uppercase text-[var(--color-text-muted)] mb-2">
                  <FontAwesomeIcon icon={faSnowflake} className="text-[0.6rem] text-[var(--color-cyan)]" />
                  Soğutucu — {filtered.length} sonuç
                </label>
                <select
                  value={selected?.id ?? ""}
                  ...
                  <div className="animate-fade-in mt-4 bg-[var(--color-bg-raised)] border border-[var(--color-cyan-dim)] border-l-2 border-l-[var(--color-cyan)] p-4">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        ...
                        { label: "TDP Desteği", value: `${selected.tdpW}W` },
                        { label: "Uyumlu Soketler", value: selected.compatibleSockets },
                        ...(selected.heightMm != null
                          ? [{ label: "Yükseklik", value: `${selected.heightMm} mm` }]
                          : []),
                        ...(selected.radiatorSizeMm != null
                          ? [{ label: "Radyatör", value: `${selected.radiatorSizeMm} mm` }]
                          : []),
                      ].map(({ label, value }) => (
