...
import usePsuSelect from "../../../hooks/usePsuSelect";
        ...
        {isLoading || !filters ? (
          ...
          <div className="grid grid-cols-[280px_1fr] gap-6 items-start">
            ...
            <div className="panel accent-top animate-fade-in-up p-5">
              ...
              <CheckGroup
                label="Verimlilik"
                options={options.efficiencyRatings}
                selected={filters.efficiencyRatings}
                onToggle={(v) => toggleArrayFilter("efficiencyRatings", v)}
              />
              <CheckGroup
                label="Modüler"
                options={options.modularTypes}
                selected={filters.modularTypes}
                onToggle={(v) => toggleArrayFilter("modularTypes", v)}
              />
              <RangeRow
                label="Watt"
                min={options.minWattage}
                max={options.maxWattage}
                value={filters.wattageRange}
                onChange={(v) => updateFilter("wattageRange", v)}
                format={(n) => `${n}W`}
              />
            </div>
