...
import useGpuSelect from "../../../hooks/useGpuSelect";
          ...
          <div className="grid grid-cols-[280px_1fr] gap-6 items-start">
            ...
            <div className="panel accent-top animate-fade-in-up p-5">
              ...
              <RangeRow
                ...
                onChange={(v) => updateFilter("memoryRange", v)}
                format={(n) => `${n} GB`}
              />
              <RangeRow
                label="TDP (W)"
            ...
            <div className="flex flex-col gap-4">
              <div className="panel accent-top corner-brackets animate-fade-in-up stagger-2 p-6">
                <label className="font-mono flex items-center gap-[6px] text-[0.65rem] tracking-[0.14em] uppercase text-[var(--color-text-muted)] mb-2">
                  <FontAwesomeIcon icon={faDisplay} className="text-[0.6rem] text-[var(--color-cyan)]" />
                  Ekran Kartı — {filtered.length} sonuç
                </label>
                <select
                  value={selected?.id ?? ""}
                ...
                >
                  ...
                  {filtered.map((item) => (
                    ...
                    >
                      {item.brand} {item.model} — {item.memoryGb} GB · TDP: {item.tdp}W · {item.lengthMm}mm
                    </option>
                  ))}
                </select>
                    ...
                    <div className="flex items-center gap-[14px] mb-[10px]">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="font-mono text-[0.58rem] text-[var(--color-cyan)] tracking-[0.08em]">
                          {selected.memoryGb} GB VRAM
                        </div>
                      </div>
                    </div>
                    <div className="grid grid-cols-4 gap-2">
                      {[
                        { label: "VRAM", value: `${selected.memoryGb} GB` },
                        { label: "Boost Saati", value: `${selected.boostClock} MHz` },
                        { label: "TDP", value: `${selected.tdp}W` },
                        { label: "Uzunluk", value: `${selected.lengthMm} mm` },
                      ].map(({ label, value }) => (
